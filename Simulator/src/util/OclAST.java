...
public class OclAST extends AbstractVisitor<String, EClassifier, EOperation, EStructuralFeature, EEnumLiteral, EParameter, EObject, CallOperationAction, SendSignalAction, Constraint> {

  static Environment<?, EClassifier, EOperation, EStructuralFeature, EEnumLiteral, EParameter, EObject, CallOperationAction, SendSignalAction, Constraint, ?, ?> env = null;
  ...
  public static LinkedList<EEnum> allEnumerations;
  ...
  public static void UMLTOECORE(EPackage pck)
  {   //fix all classes
	  allClasses = (LinkedList<EClass>) reader.getClasses(pck);
	  for (int i = 0; i < listeEClass.size(); i++) {
		  String classNmae=listeEClass.get(i).getName();
		  int j=0;
		  boolean find=false;
		  while (j<allClasses.size()&&find==false)
		  {
			  if(allClasses.get(j).getName().equals(classNmae))
			  {  
				  listeEClass.set(i, allClasses.get(j));
				  find=true;
			  }
			  else j++;
		  }
	  ...
	  //fix context
      orderedListe.set(0, getClassByName(allClasses, ((EClass)orderedListe.get(0))));
      //fix allOperation
      listeEAllOperation=(LinkedList<EOperation>) reader.getAllOperations(pck);
      
      for (int i = 1; i < orderedListe.size(); i++) {
    	  EObject obj = orderedListe.get(i);
          
    	  if(obj instanceof EAttribute)
    	  {   EAttribute att = (EAttribute) obj;
    		  EClass c= (EClass) att.eContainer(); 
    		  EClass rempC= getClassByName(allClasses, c);
    		  orderedListe.set(i, getAttributeByName(rempC, att)); 
    		  
    	  }
    	  
    	  if(obj instanceof EReference)
    	  {EReference ref = (EReference) obj;
    	   EClass c= (EClass) ref.eContainer();
    	   EClass rempC= getClassByName(allClasses, c);
    	   orderedListe.set(i, getReferenceByName(rempC, ref)); 
  ...
  public static EEnum getEnumerationByName(LinkedList<EEnum> liste, String ch)
  {
	  EEnum res=null;
	  int i=0;
	  boolean find = false;
  ...
  public static org.eclipse.uml2.uml.Class getClassByNameUML(LinkedList<org.eclipse.uml2.uml.Class> liste, String ch)
  {
	  org.eclipse.uml2.uml.Class res=null;
	  int i=0;
	  boolean find = false;
	  while(i<liste.size()&&find==false)
	  {
  ...
  public static EClass getClassByName(LinkedList<EClass> liste, String ch)
  {
	  EClass res=null;
	  int i=0;
	  boolean find = false;
...
public static OclAST getInstance() {
  ...
  public static OclAST getInstance(
	  ...
	  return new OclAST(env);
  }  
  

  @SuppressWarnings("static-access")
protected OclAST(Environment<?, EClassifier, EOperation, EStructuralFeature, EEnumLiteral, EParameter, EObject, CallOperationAction, SendSignalAction, Constraint, ?, ?> env) {
	    this.env = env;
  ...
  public static void addEClass (LinkedList<EClass> liste, EClass temp)
  {
	  if(!liste.contains(temp))
	  {
		  liste.add(temp);
		  orderedListe.add(temp);
	 // LinkedList<EClass> subTypes= (LinkedList<EClass>) reader.getClassSubtypes(allClasses, temp);
	  // Some of the classes do not have subtypes 
	//  if (subTypes !=null)
	//  for (int i = 0; i < subTypes.size(); i++) 
		//  addEClass(liste, subTypes.get(i));
		
	
	  }
  }

  public static void addEReference (LinkedList<EReference> liste, EReference temp)
  {
	  if(!liste.contains(temp))
	  {
		  liste.add(temp);
  ...
  public static void addEAttribute (LinkedList<EAttribute> liste, EAttribute temp)
  {
	  if(!liste.contains(temp))
	  {
		  liste.add(temp);
  ...
  public String visitVariableExp(VariableExp<EClassifier, EParameter> v) {
    //to investigate
	  ...
	  if(v.getType() instanceof EClass)
		addEClass(listeEClass,(EClass) v.getType());
  ...
  protected String handleVariable(Variable<EClassifier, EParameter> variable, String initResult) {
	 ...
	 System.out.println(variable.getType());
	  if(variable.getType() instanceof EClass)
			addEClass(listeEClass,(EClass) variable.getType());
			...
			for (EAnnotation ea : annotationList)
			{
				if (ea.getSource().endsWith("Pivot"))
				{
				   ...
				   helper.createQuery(val.trim()).accept(this);
					...
					{System.err.println("Crash1");e.printStackTrace();}									    
...
@Override
  ...
  public static boolean compareOperation(EOperation op1,EOperation op2)
  {
	  ...
	  return op1.getName().trim().equals(op2.getName().trim()) && op1.getEType().getName().toString().trim().equals(op2.getEType().getName().toString().trim()) && ((EClass) op1.eContainer()).getName().trim().equals(((EClass) op2.eContainer()).getName().trim());
  }
							...
							for (EAnnotation ea : annotationList)
								if (ea.getSource().endsWith("Pivot"))
								{
								   ...
								   handelOverwrriden(op1,context1);
									...
									{System.err.println("Crash3");;e.printStackTrace();}									    
