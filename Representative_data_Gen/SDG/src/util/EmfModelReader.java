...
public class EmfModelReader implements IModelReader<Resource, EPackage, EClass, EAssociation, EAttribute, EOperation> {
  ...
  public List<EPackage> getPackages() {
	 ...
	 public EList<Generalization>  extractGeneralizations()
	 {
		 
		 EList<EObject>content = getEveryThing();
		 EList<Generalization> res = new BasicEList<Generalization>();
			for(EObject obj: content)
			{ 
				   if(obj instanceof Generalization)
					res.add((Generalization)obj);
			}
			return res;
	 }
	 
	 public EList<InterfaceRealization>  extractInterfaceRealization()
	 {
		 
		 EList<EObject>content = getEveryThing();
		 EList<InterfaceRealization> res = new BasicEList<InterfaceRealization>();
			for(EObject obj: content)
			{ 
				   if(obj instanceof InterfaceRealization)
					res.add((InterfaceRealization)obj);
			}
			return res;
	 }

	public EList<EObject>  getEveryThing()
	{  
		EList<EObject> res= new BasicEList<EObject>();
		res.addAll(GenerateSeeds.OriginalModel.allOwnedElements());
		return res;
	}
