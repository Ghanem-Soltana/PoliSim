...
public class CreateTaxpayers {
    ...
    public static HashMap<EObject, LinkedList<EAttribute>> sav_after;
	...
	protected int maxAllowedRecurison = 2;
	...
	{        //caches
		...
		sav_after = new HashMap<EObject, LinkedList<EAttribute>>();
	...
	public static void removeDuplicatEAttribute( LinkedList<EAttribute> list)
	{
		
		if(list!=null)
		{
		LinkedHashSet<EAttribute> s = new LinkedHashSet<EAttribute>(list);
		list=new LinkedList<EAttribute>();
		list.addAll(s);
		}
	}
	...
	public EObject generate(EPackage ePackage,String ruleName, EClass context, EObject callingParent, EReference callingRef,LinkedList<EObject> instances, EClass root, HashMap<EReference,LinkedList<EObject>> missing,LinkedList<EObject> visitedElements, boolean isNotLateral)
	    { 
		  
		
			EObject picked=null;
		if(callingRef!=null)
		picked=  pickAnObjectFromRef(callingRef,callingParent,null);
		if(picked!=null)
		return picked;
	
		if(callingRef!=null)
		{visitedElements.add(callingRef);
			if(callingRef.getEOpposite()!=null)
				{visitedElements.add(callingRef.getEOpposite());
				}
		}
		
		
	         boolean needAdjustement = isConflicting(context, callingRef);
	         EList<Stereotype> ref_steryotypes = new BasicEList<Stereotype>();
	         ref_steryotypes=getAllSteryotypesFromModel(callingRef);
	    	 ...
	    	 // System.out.println(choosenEClass.getName());
	    	  ...
	    	  contextIntance = ((EPackage)ePackage).getEFactoryInstance().create(choosenEClass);
	    		  ...
	    		  System.out.println("A problem occured when selecting the class");
	    		  choosenEClass=getElu(context,needAdjustement);
	    		  contextIntance = ((EPackage)ePackage).getEFactoryInstance().create(choosenEClass);
	    	  }
	    	   visitedElements.add(context);
		  	   if(context!=choosenEClass)
		    	visitedElements.add(choosenEClass);
		  	   
	    	  // update links
	        if(callingParent!=null)
	  	    if(callingRef!=null)
    	     updateLink(callingRef,callingParent,contextIntance,true);
  	    
	       // System.out.println("Chosen class = "+choosenEClass.getName());
	     ...
	     //already sorted
	    	  ...
	    	  else 
	    	  {
	    		...
	    		boolean completed = checkIsCompleted (dep,visitedElements);
	    		 //nocycle rule 
	    		if((attributs.get(i).getName().contains("amount")&&context.getName().contains("External"))|| (attributs.get(i).getName().contains("month")&&context.getName().contains("Income_Detail")))
	    			attributesAfter.add(attributs.get(i));
	    		else
	    		 if(completed)
	    		attributesNow.add(attributs.get(i));
	    	  ...
	    	  //hack references
	     		    		 ...
	     		    		 for (int i = 0; i <references.size(); i++) {
	     		    			 ...
	     		    			 if(callingRef!=null)
	     		    			  if(callingRef.getEOpposite()!=null)
	     		    				if(callingRef.getEOpposite().equals(ref))
	     		    					updateLink(ref,contextIntance,callingParent,true);
	     		    			 
	     		    			 EList<Stereotype> sters = getAllSteryotypesFromModel(ref);
	     		    			 if(ref.getUpperBound()==1)
	     		    			 {
	     		    			 if(contains(sters, "use existing"))
	     		    			 {
	     		    				 
	     		    		for (Stereotype stereotype2 : sters) {
	     		    		 	if(stereotype2.getName().equals("use existing"))
	     		    		 	{
	     		    		 		picked=  pickAnObjectFromRef(ref,callingParent,contextIntance);
	     		    		
	     		    		 		
	     		    		 		if(picked!=null)
	     		    		 		{	
	     		    		 			updateLink(ref, contextIntance, picked,true);
	     		    		 			if(ref.getEOpposite()!=null)
	     		    		 			{   updateLink(ref.getEOpposite(), picked, contextIntance,true);
	     		    		 			}
	    	  ...
	    	  //attributes of now
	     		 resolveEAttributesTesnion(attributesNow);
	          ...
	          visitedElements.add(att);
	    		 ...
	    		 resolveEAttributesTesnion(attributesAfter);
	    	  ...
	    	  try{
	    	  sav_after.put(contextIntance, attributesAfter);
	...
	//HAck 
	    	 ...
	    	 for (int i = 0; i <references.size(); i++) {
	    		     ...
	    		     EReference ref = references.get(i);
	    			// System.out.println("Ref "+ref.getName() + " of "+choosenEClass.getName());
	    			...
	    			lower=Double.valueOf(value).intValue(); 
	    			 ...
	    			 {
	    			 EList<Constraint> cons=getConstraintsFromSteryotype(ref, "multiplicity"); 
	    			 if(cons!=null)
	    			 if(cons.size()>0)
	    			 ...
	    			 shouldPass=1;
	    				 ...
	    				 shouldPass=2;
	    					...
	    					if(ref.getName().equals("children"))
	    						shouldPass = 1;
	    					
	    					if(ref.getName().equals("household")&&choosenEClass.getName().equals("Dependent"))
	    						shouldPass = 2;
	    			...
	    			//unsafe
	        		 ...
	        		 {   
	        			 ...
	        			 if(count < lower)
	        			 {
	        				 ...
	        				 if(callingRef!=null)
	        				picked=  pickAnObjectFromRef(callingRef,callingParent,contextIntance);	
	        				 ...
	        				 if(picked!=null)
	        				 {
	        					 ...
	        					 EReference theOpposite = ref.getEOpposite();
	        		        		if(theOpposite!=null)
	        		        		{ 
	        		        		updateLink(theOpposite,picked,contextIntance,true);
	        			 ...
	        			 if(count<lower)
	        			 {
	        				 ...
	        				 else 
	        				 {
	        					 ...
	        					 for (int j = 0; j < lower-count && count_Recursion.get(ref)<=maxAllowedRecurison; j++) {
									...
									condidates.add(generate(ePackage, ruleName, (EClass)ref.getEType(), contextIntance, ref, instances,root,missing,visitedElements, !ref.isMany()));
	        	   ...
	        	   if(ref.getEOpposite().equals(callingRef))
	        		...
	        		if(callingParent!=null)
	        		{
	        			if(safeEReferences.contains(callingRef)||unsafeEReferences.contains(callingRef))
	        			{
	        		updateLink(ref,contextIntance,callingParent,true);
	        		}
	        		}
	        		...
	        		if(ref.isMany())
	    		     ...
	    		     if(generate)
	    		     {
			    	 linkedToMe.add(generate(ePackage,ruleName,cible,contextIntance, ref,instances,root,missing,visitedElements, !ref.isMany()));
					...
					updateLink(theOpposite, linkedToMe, contextIntance);
	        		 ...
	        		 if(ref.isMany())
	        		 {
	        			 updateAfteInDepth(linkedToMe,ref,contextIntance);
	        	  	  ...
	        	  	  for (int xx = 0; xx <attributesAfter.size(); xx++) { 
	        	    	  ...
	        	    	  // boolean completed = checkIsCompleted (dep,visitedElements);
	        	    		 ...
	        	    		 if(completed)
	        	    		 {
	        	    			   	if(att.isDerived()==false)
	        	    	          	{   //nocycle rule 
	        	    			   		if(!att.getName().contains("birth"))
	        	    			   		handelAttributesSteryotyps(att,contextIntance,callingParent!=null?callingParent:contextIntance);
	        	    	          	}
	...
	private void updateAfteInDepth(List<EObject> list, EReference ref, EObject contextIntance) {
	
	 	    LinkedList<EAttribute> attributesAfterTemp = getAttributesForAfter(list);
	  	     for (EObject linked : list) {
      	  	  for (int xx = 0; xx <attributesAfterTemp.size(); xx++) { 
      	    	if(attributesAfterTemp.get(xx).isDerived()==false)
      	    		handelAttributesSteryotyps(attributesAfterTemp.get(xx),linked,contextIntance);
	  	   ...
	  	   for (EObject linked : list) {
	    	  LinkedList<EReference> references=getRefrences(linked.eClass());   
	    	  for (EReference eReference : references) {
	    		 if(eReference.isMany())
	    		updateAfteInDepth(((List<EObject>)linked.eGet(eReference)), eReference, linked);
			}
	  	   }
	...
	private LinkedList<EAttribute> getAttributesForAfter(List<EObject> list) {
		LinkedList<EAttribute> res = new LinkedList<EAttribute>();
		for (EObject obj : list) {
		LinkedList<EAttribute> temp = sav_after.get(obj);	
		if(temp!=null)
		if(temp.size()>0)
		res.addAll(temp);
		}
		removeDuplicatEAttribute(res);
		return res;
	}


	public  LinkedList<EAttribute> getAffterAttributes(EObject obj, LinkedList<EObject> visitedElements)
	  {
	      LinkedList<EAttribute> attributs=getReleventAttributes(obj.eClass(), all); 
  	      LinkedList<EAttribute> attributesAfter = new LinkedList<EAttribute>();
    	  for (int i = 0; i <attributs.size(); i++) { 
    	  EAttribute att = attributs.get(i);
          LinkedList<EObject> dep = new LinkedList<EObject>();
          ...
          if(dep.size()>0)
          {
    	  breakGeneralization(dep,obj.eClass());
    	  removeDuplicatEObject(dep);
          }
    	  if( !(dep.size()==0||att.isDerived()))
    	  {
...
return attributesAfter;
	  }
	  
	  
	  @SuppressWarnings("unused")
	private boolean checkIsNotlater(LinkedList<EObject> dep, EClass elemJ) {
		for (EObject depI : dep) {
			if(dep instanceof EClass)
			{
				...
				if(getAllChildrenClasses((EClass)depI).contains(elemJ))
					return false;
				else if(((EClass)depI).getESuperTypes().contains(elemJ))
					return false;
	...
	private boolean checkIsCompleted( LinkedList<EObject> dep, LinkedList<EObject> visitedElements) {
		...
		{
			EObject depI = dep.get(i);
			boolean treated = false;
			int j=0;
			while(j<visitedElements.size()&&treated==false)
			{ 
				EObject elemJ = visitedElements.get(j);
				
				if(depI == elemJ)
					treated = true;	
						 
				else {
				if(depI instanceof EClass && elemJ instanceof EClass)
				{
			
				if(getAllChildrenClasses((EClass)visitedElements.get(j)).contains(dep.get(i)))
					treated=true;
				else if(((EClass)visitedElements.get(j)).getESuperTypes().contains(dep.get(i)))
					treated=true;
				}
	...
	public int takeFromTheStack(EObject contextIntance, EClass type, int lower, EReference ref)
	  {
			 ...
			 {   
				...
				{
				//Doube update 
				updateLink(ref,contextIntance,condidate,true);
        		if(theOpposite!=null)
        		updateLink(theOpposite,condidate,contextIntance,true);
        		count ++;
				}
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public void updateLinkOneSide(EReference ref, EObject contextIntance, EObject otherObject, boolean chance)
	{	chance =false;
		try{
		if(ref.getUpperBound()==1)
   		{
   		...
   		{if(ref.getEOpposite()!=null)
   	   	 otherObject.eSet(ref.getEOpposite(), contextIntance);}


   		}
		}catch(Exception e)
		{   // System.out.println(ref.getName() +" between "+contextIntance+" and "+otherObject);
			if(chance)
			{updateLink( ref, contextIntance ,otherObject,  false);}
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public void updateLink(EReference ref, EObject contextIntance, EObject otherObject, boolean chance)
	{	chance =false;
		try{
		if(ref.getUpperBound()==1)
   		{
   		...
   		if(ref.getEOpposite()!=null)
   	 	updateLinkOneSide(ref.getEOpposite(), otherObject, contextIntance, chance);	 
	...
	public void updateLink(EReference ref, EObject contextIntance, LinkedList<EObject> otherObjects )
	{
		...
		if(ref.getEOpposite()!=null)
		for (EObject eObject : otherObjects) {
		updateLinkOneSide(ref.getEOpposite(),eObject,contextIntance,true);
		}
		...
		public EObject pickAnObjectFromRef(EReference ref,EObject parentInstance,EObject contextInstance)
		{
			...
			for (int ixx = 0; ixx < relatedSteryotypes.size() && done==false; ixx++) {
	        	...
	        	if(stereotype.getName().equals("use existing")  && done==false)
	        	{
	        		...
	        		else 
	        		{
	        			...
	        			{
	        			if(isTheContext(contextInstance, getEVersion(context)))
		        		{
		        			    OCLForJAVA.init("", contextInstance);	
		        			    Constraint c = 	(Constraint)getValue(ref, stereotype,"eligibleCondidatesQuery");
		        			    String toBeEvaluated = c.getSpecification().stringValue();	
		        			    Collection<EObject> selectable = OCLForJAVA.evaluateECollection(contextInstance, toBeEvaluated, "temp", "Physical_Person", "Set");
		    					if(selectable!=null)
		    					if(selectable.size()>0)
		    					{
		    						int index = FiledGenerator.randomIntegerRange(0, selectable.size()-1);
   ...
   return OclDependencies.attributesNeeds.get(att.toString());
	...
	public void resolveEReferencesTesnion(LinkedList<EReference> references)
	{
	   ...
	   for (int i = 0; i < references.size()-1; i++) {
		    ...
		    for (int j = i+1; j < references.size(); j++) {
			   ...
			   if(posI > posJ)
			   {Collections.swap(references, i, j);
	...
	public int getPosInOrder(String name)
	{
		...
		CreateTaxpayers.orderIterator = new TopologicalOrderIterator<String, DefaultEdge>(GenerateSeeds.g);
	
		 while (orderIterator.hasNext()) {
				
			 res++;
			 v = orderIterator.next();
			 if(v.trim().equals(name.trim()))
	...
	public boolean requires (EAttribute attI, EAttribute attJ)
	{
	    ...
	    if(attI.getName().equals("disability_percentage") && attJ.getName().equals("disability_type"))
	    	return true;
	    String cut = cache_requires.get(attI.toString()+"#"+attJ.toString());
	    if(cut!=null)
	    return cut.equals("true");
	    LinkedList<EObject> temp = getNeedsForAtt(attI); 
	    if(temp==null)
	        		...
	        		if(context!=null && !ref.getEType().getName().equals(context.getName()) && ref.getEContainingClass().getName().equals(context.getName()))
	        		{
	        			...
	        			for (int i = 0; (i < constraints.size())&&(done==false); i++) {
		        			...
		        			container.add(constraint);
	   	    				...
	   	    				if(res.equals("null")||res.equals("invalid"))
	        				{res="Non";
	        				done =false;
	        				}
	        				}
 ...
 if(eClass.getName().equals("Tax_Payer"))
	...
	{ 
		...
		{ 
			...
			{
				if(minimum.equals("invalid"))
					minimum = maximum;
					else maximum = minimum;
			}
		}
		
		if(minimum.equals("null")||maximum.equals("null"))
		{ 
			if(minimum.equals("null")&&maximum.equals("null"))
			{minimum="0";maximum="0";}
			else 
	...
	{ 
        ...
        EList<Stereotype> relatedSteryotypes = sortSteryotypes(getAllSteryotypesFromModel(att));
        Stereotype stereotype=null;
		for (int ixx = 0; ixx < relatedSteryotypes.size() && res==false; ixx++) {        	 stereotype=relatedSteryotypes.get(ixx);   	
			...
			if(stereotype.getName().equals("fixed value")&& res==false)
			{
				...
				else 
				{
					...
					if(evaluation.equals("invalid"))
					{if(isTheContext(contextIntance, context))
					{
					OCLForJAVA.init("", contextIntance);
					evaluation= OCLForJAVA.evaluateString(contextIntance, value, "evaluation");
					...
					{   if(isTheContext(contextIntance, context))
						...
						if(evaluation.equals("invalid"))
							{
							//correctMissingRef(contextIntance,parentInstance,att);
						    evaluation= OCLForJAVA.evaluateString(contextIntance, value, "evaluation");
						    if(evaluation.equals("invalid"))
				...
				else
				{
					...
					EClass context = getEVersion(contextTemp);
					 ...
					 if(isTheContext(contextIntance, context))
						...
						String tempV = generateValue (att,minEvaluation,maxEvaluation);

						updateAttribute(att, contextIntance, parentInstance, castValue(att,tempV));
						res=true;
						}
					 else 
					 {
			...
			{  /* 
				...
				res=true;
				last = last +1;
				updateAttribute(att, contextIntance, parentInstance, castValue(att,String.valueOf(last)));
	...
	@SuppressWarnings("unchecked")
	public boolean handelAttributesSteryotypsAfter1(EAttribute att, EObject contextIntance,EObject parentInstance)
	{ 
		boolean res=false;
        EList<Stereotype> relatedSteryotypes = sortSteryotypes(getAllSteryotypesFromModel(att));
        Stereotype stereotype=null;
	...
	*/
	...
	public static String evaluateQueryClean (EObject instance, String queryString)
	{
		String res="false";

		try {
			res = OCLForJAVA.evaluateStringClean(instance,queryString);
			//System.out.println("context==>"+OclAST.helper.getContextClassifier());
			//System.out.println("Self==>"+instance);
			//System.out.println("Query==>"+queryString);
			//System.out.println("Res==>"+res);
