...
public class EClassUMLAdapter extends EClassAdapter<Class> implements EClassifier{
	...
	public EList<EReference> getEReferences() {
		...
		for (Association ass : asList){
			for (Property pro : ass.getMemberEnds()){
	...
	public EStructuralFeature getEStructuralFeature(String featureName) {
		...
		EList<EStructuralFeature> all = getEStructuralFeatures();
		boolean find =false;
		int i=0;
		while(i<all.size()&&find==false)
	    if(all.get(i).getName().equals(featureName))
	    	find=true;
	    else i++;
		
		if(find) return all.get(i);
		else return null;
